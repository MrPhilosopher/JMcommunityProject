...
import { MasjidFormData, MasjidType, PARISHES, MasjidWithRelations } from '@/types/masjid';
...
export default function EditMasjidPage() {
  ...
  useEffect(() => {
    if (params.id) {
      Promise.all([fetchMasjid(), fetchMembers()]);
    }
  }, [params.id]);
  ...
  const fetchMasjid = async () => {
    try {
      ...
      const masjid: MasjidWithRelations = response.data;
      ...
      reset({
        ...
        imam_id: masjid.imam_id || undefined,
        established_year: masjid.established_year || undefined,
      ...
      if (masjid.imam) {
        setMemberSearch(masjid.imam.legal_name);
      }
      
      if (masjid.shura_members) {
        setSelectedShuraMembers(masjid.shura_members.map(m => m.id));
      }
    } catch (error) {
  ...
  const onSubmit = async (data: MasjidFormData) => {
    try {
      ...
      await api.put(`/masjids/${params.id}`, submitData);
      router.push('/masjids');
    } catch (error) {
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-4xl">
        <h1 className="mb-6 text-3xl font-bold text-gray-900">Edit Masjid</h1>
