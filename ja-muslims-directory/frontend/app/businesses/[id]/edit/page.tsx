...
import { Member, Business } from '@/types';
...
export default function EditBusinessPage() {
  ...
  const fetchBusiness = async () => {
    try {
      ...
      reset({
        ...
        halal_certified: business.halal_certified,
        accepts_zakat: business.accepts_zakat,
        social_media: business.social_media || '',
        is_active: business.is_active,
        notes: business.notes || '',
      });

      // Set the owner name in search
      if (business.owner_name) {
        setMemberSearch(business.owner_name);
      }
    } catch (error) {
  ...
  const onSubmit = async (data: BusinessForm) => {
    try {
      ...
      await api.put(`/businesses/${params.id}`, payload);
      router.push(`/businesses/${params.id}`);
    } catch (error: any) {
      console.error('Error updating business:', error.response?.data || error);
